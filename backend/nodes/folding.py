...
from backend.nodes.common import ExecutionContext, copy_paths_as_artifacts, node_dir, option, parse_score_files, payload_from_artifacts, read_payload_files, scores_to_rows
...
async def rosetta_fold(ctx: ExecutionContext, node: WorkflowNode, inputs: dict[str, TypedPayload]) -> dict[str, TypedPayload]:
    ...
    fasta_path = work_dir / "rf3_input.fasta"
    fasta_path.write_text(write_fasta(sequences))
    ...
    structure_paths, score_paths = await run_rf3_fold(
        run_id=ctx.run_id,
        node_id=node.id,
        node_type=node.type,
        work_dir=work_dir,
    ...
    scores = parse_score_files(score_paths)
    if not scores:
    ...
    return {
        ...
        "score": payload_from_artifacts("Score", [score_json, score_csv], data=scores),
    }
